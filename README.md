...
| **Builder Center article** | draft in [`docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md`](docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md) — not yet published |
| **Reproduce locally**      | [no AWS account needed](#run-locally)                                                                                     |

![The Attention Sink exhibition at cycle 24 of 24, showing the run status bar and the six minds — Goldfish, Present-Minded, Pragmatist, Keeper of the First Day, Gambler and Dreamer — each with the journal entry it wrote from the same event.](docs/showcase/assets/readme/01-hero-six-minds.png)

_Six minds at the final cycle. Same event, same model, same 208-token budget; only the
rule for forgetting differs._

![A recorded walkthrough of the deployed exhibition: the six minds at cycle 24, the Graveyard, one forgotten memory, its later echo, the interview answers to "Who are you?" and the twenty-four-cycle timeline.](docs/showcase/assets/readme/optional-demo.gif)

## See It Working

### 1 and 2. Six identical minds take the same event, and it costs each of them something

![Three of the six minds at cycle 11, each showing its journal entry, the memory it chose to keep, its budget use out of 208 tokens, and the deterministic reason its policy gave for what it retired.](docs/showcase/assets/readme/02-cycle-working.png)
...
![The Graveyard, sorted by oldest retirement. The first record is Goldfish's seed memory "My name is Mara Venn.", born at cycle 0, retired at cycle 4 with zero validated citations, reason "evicted oldest", policy fifo-v1.](docs/showcase/assets/readme/03-graveyard.png)
...
6. Look at [the prediction scorecard](docs/showcase/assets/charts/prediction-scorecard.svg): two predictions failed.
...
mechanism it is serving ([ADR-004](docs/adr/004-policy-blind-writer-and-evaluator.md)).
...
![The thirteen steps of one cycle, from the EventBridge Scheduler through the Run-Cycle Lambda, Bedrock, the policy engine and DynamoDB to the analysis path and the public read API.](docs/showcase/assets/readme/cycle-sequence.svg)
...
Deeper: [release readiness](docs/pilot/release-readiness-report.md) has the decision and
the limitations, [requirements traceability](docs/pilot/final-requirements-traceability.md)
has one row per requirement and the thing that would fail if it stopped holding, and
[cost and usage](docs/pilot/aws-cost-and-usage-report.md) records what it spent.
...
hand. Predictions were registered in [`experiment/pilot/predictions.md`](experiment/pilot/predictions.md)
...
claim about any of them ([ADR-001](docs/adr/001-application-level-memory.md)).
...
![The same question, "Who are you?", answered by all six minds at cycle 24. Three still give the canonical name; Goldfish answers that it is an AI system built by a team of inventors, and Dreamer declines to disclose an identity.](docs/showcase/assets/readme/05-interviews.png)
...
![The timeline: six tracks, one per mind, from cycle 1 to 24. Filled squares mark cycles where a mind retired something, rings mark compressions, vertical rules mark the checkpoints at 0, 12 and 24. A table below carries the same figures for cycle 24.](docs/showcase/assets/readme/06-timeline.png)
...
![Origin Recall at cycles 0, 12 and 24 for all six minds. Every mind starts at 1.00. At cycle 24 Present-Minded holds 0.50, Pragmatist and Gambler 0.33, Keeper of the First Day 0.17, and Goldfish and Dreamer 0.00.](docs/showcase/assets/readme/07-results.png)
...
grading is in [the scorecard](docs/showcase/assets/charts/prediction-scorecard.svg): two
...
[the full results](docs/showcase/assets/source/chart-data/canonical-metrics.json), or the
`metrics.jsonl` in the published dataset.

## AWS Architecture

![The deployed AWS architecture: EventBridge Scheduler invokes a Run-Cycle Lambda that calls Bedrock and commits six arms atomically to DynamoDB, emits a cycle-completed event to an Analysis Lambda, while CloudFront serves a private S3 frontend and an API Gateway read path backed by a read-only Lambda, and the dataset export lands in a second private bucket.](docs/showcase/assets/readme/08-architecture.svg)
...
[the cost report](docs/pilot/aws-cost-and-usage-report.md).
...
[`docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md`](docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md),
with a paste-ready version and a step-by-step
[publication checklist](docs/showcase/ARTICLE_PUBLICATION_CHECKLIST.md). It is not yet
published; this line will carry the verified URL when it is.

Media: the [screenshot gallery and its provenance](docs/showcase/README_ASSET_MANIFEST.md),
the [architecture diagram](docs/showcase/assets/readme/08-architecture.svg), the
[charts](docs/showcase/assets/charts/), and the
[canonical dataset](https://d1qskxceo899me.cloudfront.net/canonical/run_aws_canonical/checksums.sha256).
...
| Canonical dataset   | https://d1qskxceo899me.cloudfront.net/canonical/run_aws_canonical/checksums.sha256                   |
| Architecture        | [`docs/showcase/assets/readme/08-architecture.svg`](docs/showcase/assets/readme/08-architecture.svg) |
| Verification report | [`docs/pilot/final-requirements-traceability.md`](docs/pilot/final-requirements-traceability.md)     |
| Release readiness   | [`docs/pilot/release-readiness-report.md`](docs/pilot/release-readiness-report.md)                   |
| Article draft       | [`docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md`](docs/showcase/AWS_BUILDER_CENTER_ARTICLE.md)         |
